...
const fallbackDiary = {
  diary:
    '今天你一个人走过了老街入口、夜市街区。一开始有点犹豫，但你还是慢慢做出了下一步选择，也把这趟旅行认真记录了下来。',
...
function DiaryPage() {
  ...
  const visitedPlaceNames = useMemo(
    () =>
      state.visitedPlaces?.length
        ? state.visitedPlaces.map(toDisplayText).filter(Boolean)
        : ['老街入口', '夜市街区'],
    [state.visitedPlaces],
  )
  ...
  const unlockedBadgeNames = unlocked.length ? unlocked.map((badge) => toDisplayText(badge)) : badgeNames
    ...
    <section className="page">
      ...
      {!isLoading && diaryData?.summary_tags?.length ? (
        <article className="diary-card">
          <Tags size={22} />
          <h2>今日标签</h2>
          <p>{diaryData.summary_tags.join(' / ')}</p>
        </article>
      ) : null}
