...
const normalizeDemoImageName = (payload = {}) => String(payload.image || payload.file?.name || '').trim().toLowerCase()

const buildDemoPhotoResponse = (payload = {}) => {
  const imageName = normalizeDemoImageName(payload)
  const demoMap = {
    '图片_草原.jpg': {
      ...
      task_result: { passed: true, reward_badge: '自然旷野碎片', reason: '照片有明确的旅行现场和自然环境信息，适合作为本次演示的识图分享素材。' },
      memory_fragment: {
    ...
    '图片_大海.jpg': {
      ...
      task_result: { passed: true, reward_badge: '海风记忆碎片', reason: '照片包含清晰的大海和旅行目的地线索，适合生成分享文案和日记素材。' },
      memory_fragment: {
    ...
    '图片_日落.jpg': {
      ...
      task_result: { passed: true, reward_badge: '黄昏收束碎片', reason: '照片有明确的日落氛围和时间线索，适合作为当天日记生成的视觉素材。' },
      memory_fragment: {
  ...
  const match = demoMap[imageName]
  return match ? { ...match, demo_fixture: true, demo_image_name: imageName } : null
}
...
const buildMockPhotoResponse = (payload = {}) => {
  const demoResponse = buildDemoPhotoResponse(payload)
...
const buildPhotoPayload = (payload = {}) => ({
  ...
  image: payload.image || payload.file?.name || '',
...
export const analyzePhoto = async (payload = {}) => {
  if (payload.file) {
    ...
    formData.append('image_name', payload.image || payload.file.name || '')
    formData.append('photo_mode', payload.photo_mode || '')
